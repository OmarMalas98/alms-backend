...
class TextAreaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $textArea->save();
        return response()->json(['message' => 'textArea created successfully',"text-area" => $textArea->component], 201);
    }
    ...
    public static function update(Request $request,$component)
    {
        ...
        if ($request->has('order')) {
            ...
            $textArea->component->order =$request->order;
        }
        if($request->has('body')) {
            $textArea->body = $request->body;
            $textArea->save();
            $request->request->remove('body');
        }
            $component->update($request->all());
            return response()->json(['message'=>"textArea updated successfully"]);
    }
